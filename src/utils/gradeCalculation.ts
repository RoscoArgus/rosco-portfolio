...
export const calculateClassification = (yearlyGrades: DegreeYear[]) => {
  ...
  const degreeAverage = yearlyGrades?.reduce(
    (acc, year) => acc + calculateYearlyGrade(year) * (year.weight / TOTAL_WEIGHT),
    0
  );
...
export const calculateOverallGPA = (yearlyGrades: DegreeYear[]) => {
  ...
  const overallGPA = yearlyGrades?.reduce(
    (acc, year) => acc + calculateYearlyGPA(year) * (year.weight / TOTAL_WEIGHT),
    0
  );
...
export const calculateOverallPercentage = (yearlyGrades: DegreeYear[]) => {
  ...
  const overallPercentage = yearlyGrades?.reduce(
    (acc, year) => acc + calculateYearlyPercentage(year) * (year.weight / TOTAL_WEIGHT),
    0
  );
